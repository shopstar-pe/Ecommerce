...
{
    public class SaleOrderInProgressListQuery : IRequest<PagedViewModelResult<SaleOrderListViewModel>>
    {
        ...
        public class Handler : IRequestHandler<SaleOrderInProgressListQuery, PagedViewModelResult<SaleOrderListViewModel>>
        {
            ...
            public async Task<PagedViewModelResult<SaleOrderListViewModel>> Handle(SaleOrderInProgressListQuery request, CancellationToken cancellationToken)
            {
                var tenantId = this._userIdentityService.GetTenantId();

                var orderStatus = new List<SaleOrderStatus> { };
                orderStatus.Add(SaleOrderStatus.InTransit);
                orderStatus.Add(SaleOrderStatus.PartialDelivered);
                orderStatus.Add(SaleOrderStatus.ReadyToPickUp);
