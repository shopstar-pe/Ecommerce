...
using Shippings.Application.Commands.DriverCommand;
...
namespace Shippings.API.Controllers
{
    ...
    public class DriversController : Controller
    {
        ...
        /// <returns></returns>
        [HttpGet]
        [ProducesResponseType(typeof(PagedViewModelResult<DriverListViewModel>), StatusCodes.Status200OK)]
        [ProducesDefaultResponseType]
        [Consumes("application/json")]
        public async Task<IActionResult> Get([FromQuery]DriverListQuery query) {
        ...
        [ProducesResponseType(typeof(DriverViewModel), 200)]
        [ProducesDefaultResponseType]
        [Consumes("application/json")]
        public async Task<IActionResult> GetById(Guid id)
        {
            var model = await this._mediator.Send(new DriverDetailQuery() { Id = id });
        ...
        [ProducesDefaultResponseType]
        [Consumes("application/json")]
        public async Task<IActionResult> Put(int id, [FromBody]UpdateDriverCommand command)
        {
            if (!this.ModelState.IsValid)
                return this.BadRequest(this.ModelState);
        ...
        {
            var response = await this._mediator.Send(new DeleteDriverCommand { DriverId = id });
